...
// frappe.ready(function() {
	...
	$('.go_to_background').click( () => {
	    ...
	    console.log(living)
	    console.log(reason)
	    console.log(guardianProof)
	    var parentProvided = false;
	    var guardianProvided = false;
	    ...
	    if(living == "no") {
    	    if (guardian_name != "" && relationship != "" && guardian_number != "" && reason != "" && guardianProof != "") {
    	        guardianProvided = true;
    	    }
    	    else {
    	        guardianProvided = false;
    	    }
	    }
	    else if (living == "yes") {
	        guardianProvided = true;
	    ...
	    console.log(guardianProvided)
	    
        if (first_name == "" || last_name == "" || middle_name == "" || gender == null || birthday == "" || mother_tongue == "" || ethnic == null || religion == null || mobile_number == "" || home_phone == "" || email == "" || address == "" || barangay == null || city == null || province == null || parentProvided == false || guardianProvided == false) {
            ...
            frappe.msgprint({
                title: __('Error'),
                indicator: 'red',
                message: __('Some required fields have an incorrect format, please fill them up properly before submission.')
            });
        }
	...
	$('#gs_add_button').click( () => {
		if(Gs_School_name === "" || Gs_Year_started === "" || Gs_Year_end === "" || Gs_Province === "" || Gs_city === "" || GsStarted == "" || GsEnded == ""){
	...
	$('#college_gs_add_button').click( () => {
		if(college_gs_school_name === "" || college_gs_year_start === "" || college_gs_year_end === "" || college_gs_province === "" || college_gs_city === "" || college_gs_started === "" || college_gs_ended === ""){
			frappe.throw('<p class="text-red">Please fill all values carefully</p>');
		}else{
			let my_object = {}
	...
	$('#college_jhs_add_button').click( () => {
		if(college_jhs_school_name === "" || college_jhs_year_start === "" || college_jhs_year_end === "" || college_jhs_province === "" || college_jhs_city === "" || college_jhs_started === "" ||  college_jhs_ended === ""){
			frappe.throw('<p class="text-red">Please fill all values carefully</p>');
		}else{
			let my_object = {}
	...
	$('#college_shs_add_button').click( () => {
		if(college_shs_school_name === "" || college_shs_year_start === "" || college_shs_year_end === "" || college_shs_province === "" || college_shs_city === "" || college_shs_started === "" || college_shs_ended === ""){
			frappe.throw('<p class="text-red">Please fill all values carefully</p>');
		}else{
			let my_object = {}
	...
	$('#shs_gs_add_button').click( () => {
		if(shs_gs_school_name === "" || shs_gs_year_start === "" || shs_gs_year_end === "" || shs_gs_province === "" || shs_gs_city === "" || shs_gs_gradeStarted  === "" || shs_gs_gradeEnded === ""){
			frappe.throw('<p class="text-red">Please fill all values carefully</p>');
		}else{
			let my_object = {}
	...
	$('#shs_jhs_add_button').click( () => {
		if(shs_jhs_school_name === "" || shs_jhs_year_start === "" || shs_jhs_year_end === "" || shs_jhs_province === "" || shs_jhs_city === "" || shs_jhs_gradeStarted == "" || shs_jhs_gradeEnded == ""){
			frappe.throw('<p class="text-red">Please fill all values carefully</p>');
		}else{
			let my_object = {}
...
function getAge(dateString) 
{
	...
	$('.continue_to_confirmation').click( () => {
		...
		console.log("acad" + acad_value  +academic_department)
		let grade_level = educational_level + " - " + program
		console.log(educational_level);

		const acad_department = educational_level;
				...
				<div class="row">
						<div class="col-md-10 row">
							...
							<div class="col-md-3">
								...
								<p class="text-gray text12 p-0">${res.data.academic_department}</p>
							</div>
							<div class="col-md-3">
						...
						<div class="col-md-10 row mt-5">
							<div class="col-md-3">
								...
								<p class="text-gray text12 p-0">${res.data.academic_department + "-" + program}</p>
							</div>
							<div class="col-md-3 ">
    ...
    frappe.call({
        ...
        callback: function (r) {
            setTimeout(function () {
                ...
                fetch('https://eas.test.pccr.edu.ph/api/resource/Email Queue?filters=[["communication","=", "' + comm + '"]]')
                    .then(response => response.json())
                    .then(data => {
